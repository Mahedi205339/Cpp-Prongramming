...
void loadingAnimation(string message) {
    ...
    for (int i = 0; i < 3; ++i) {
        cout << ".";
        this_thread::sleep_for(chrono::milliseconds(500));  // Simulate loading delay
    }
    cout << "\n";
}
...
void viewTrains(const vector<Train> &trainList) {
    if (trainList.empty()) {
        ...
        loadingAnimation("Loading Train Schedule");
...
void searchTrainByNumber(const vector<Train> &trainList) {
    ...
    loadingAnimation("Searching for Train");
    bool found = false;
    for (const Train &train : trainList) {
